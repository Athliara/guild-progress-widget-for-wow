...
final class Athlios_Guild_Progress_Plugin
{
    private const PAGE_SLUG = 'guild-progress-widget-for-wow';
    private const PLUGIN_LABEL = 'Guild Progress Widget for WoW';
    private const MENU_LABEL = 'Guild Progress';
    ...
    private function __construct()
    {
        ...
        add_filter('plugin_action_links_' . plugin_basename(ATHLIOS_GUILD_PROGRESS_PLUGIN_FILE), array($this, 'add_plugin_action_links'));
    }

    public function register_shortcode(): void
    {
        add_shortcode('wow_guild_progress', array($this, 'render_shortcode'));
    }

    public function register_widget(): void
    {
        register_widget('Athlios_Guild_Progress_Widget');
    }
    ...
    public function enqueue_assets(): void
    {
        wp_register_style(
            'wgp-style',
            ATHLIOS_GUILD_PROGRESS_PLUGIN_URL . 'assets/wgp.css',
            array(),
            filemtime(ATHLIOS_GUILD_PROGRESS_PLUGIN_DIR . 'assets/wgp.css')
        );

        wp_register_script(
            'wgp-script',
            ATHLIOS_GUILD_PROGRESS_PLUGIN_URL . 'assets/wgp.js',
            array(),
            filemtime(ATHLIOS_GUILD_PROGRESS_PLUGIN_DIR . 'assets/wgp.js'),
            true
        );
    }
        ...
        if (empty($settings['client_id']) || empty($settings['client_secret']) || empty($settings['realm_slug']) || empty($settings['guild_slug'])) {
            update_option(
                ...
                array(
                    'updated_at' => time(),
                    'raids' => array(),
                    'error' => 'Configure Battle.net credentials and guild details in Guild Progress Widget for WoW settings.',
                ),
                false
            );
            return;
    ...
    private function get_raid_background_url(string $raidSlug): string
    {
        ...
        foreach (array('jpg', 'png', 'webp') as $ext) {
            foreach ($slugCandidates as $candidate) {
                ...
                if (file_exists(ATHLIOS_GUILD_PROGRESS_PLUGIN_DIR . $relative)) {
                    return ATHLIOS_GUILD_PROGRESS_PLUGIN_URL . $relative;
                }
            }
        }
    ...
    public function enqueue_admin_assets(string $hook_suffix): void
    {
        ...
        wp_enqueue_style('wgp-admin-settings', ATHLIOS_GUILD_PROGRESS_PLUGIN_URL . 'assets/admin-settings.css', array(), ATHLIOS_GUILD_PROGRESS_PLUGIN_VERSION);
        wp_enqueue_script('wgp-admin-settings', ATHLIOS_GUILD_PROGRESS_PLUGIN_URL . 'assets/admin-settings.js', array(), ATHLIOS_GUILD_PROGRESS_PLUGIN_VERSION, true);
        wp_localize_script(
            'wgp-admin-settings',
            'wgpAdminSettings',
            array(
        ...
        <div class="wrap wgp-admin-page">
            ...
            <form method="post" action="options.php" class="wgp-admin-form">
                ...
                <div class="wgp-admin-tabs" role="tablist" aria-label="<?php esc_attr_e('Guild Progress Widget for WoW settings', 'guild-progress-widget-for-wow'); ?>">
                    <button type="button" class="wgp-admin-tab is-active" data-tab="guild-import"><?php esc_html_e('Guild Import', 'guild-progress-widget-for-wow'); ?></button>
                    <button type="button" class="wgp-admin-tab" data-tab="appearance"><?php esc_html_e('Appearance', 'guild-progress-widget-for-wow'); ?></button>
                    <button type="button" class="wgp-admin-tab" data-tab="manual-override"><?php esc_html_e('Manual Override', 'guild-progress-widget-for-wow'); ?></button>
                </div>
                ...
                <section class="wgp-admin-panel is-active" data-panel="guild-import">
                    <div class="wgp-admin-card">
                        <div class="wgp-admin-field-grid wgp-admin-field-grid--compact">
                            <?php $this->render_text_field('guild_name', __('Guild Name', 'guild-progress-widget-for-wow'), (string) $settings['guild_name'], __('Enter the World of Warcraft guild name.', 'guild-progress-widget-for-wow')); ?>
                            <?php $this->render_text_field('server', __('Realm', 'guild-progress-widget-for-wow'), (string) $settings['server'], __('Enter the World of Warcraft realm name.', 'guild-progress-widget-for-wow')); ?>
                            <?php $this->render_region_field((string) $settings['region'], $resolvedLocale); ?>
                        </div>
                        <div class="wgp-admin-field-grid wgp-admin-field-grid--compact">
                            <?php $this->render_text_field('client_id', __('Battle.net Client ID', 'guild-progress-widget-for-wow'), (string) $settings['client_id'], __('Create API credentials in the Battle.net Developer Portal: develop.battle.net.', 'guild-progress-widget-for-wow')); ?>
                            <?php $this->render_text_field('client_secret', __('Battle.net Client Secret', 'guild-progress-widget-for-wow'), (string) $settings['client_secret'], __('Keep this private. It is used together with Client ID for Blizzard API authentication.', 'guild-progress-widget-for-wow'), 'password', array('autocomplete' => 'new-password')); ?>
                        </div>
                        ...
                        <div class="wgp-admin-actions">
                            ...
                            <button type="submit" class="button button-secondary" form="wgp-manual-sync-form"><?php esc_html_e('Sync Guild Data', 'guild-progress-widget-for-wow'); ?></button>
                        </div>
                        <?php if ($settingsUpdated !== '') : ?>
                            <div class="wgp-inline-notice wgp-inline-notice--success"><p><?php esc_html_e('Settings saved.', 'guild-progress-widget-for-wow'); ?></p></div>
                        <?php endif; ?>
                    ...
                    <div class="wgp-admin-card">
                        <div class="wgp-admin-field-grid wgp-admin-field-grid--list">
                            <div class="wgp-admin-field wgp-admin-field--raids">
                                <span class="wgp-admin-field__label"><?php esc_html_e('Enabled Raids', 'guild-progress-widget-for-wow'); ?></span>
                                ...
                                <p class="description"><?php esc_html_e('If none are selected, the widget falls back to the latest expansion raids.', 'guild-progress-widget-for-wow'); ?></p>
                            </div>
                        </div>
                        <div class="wgp-admin-actions"><?php submit_button(__('Save Changes', 'guild-progress-widget-for-wow'), 'primary', 'submit', false); ?></div>
                        <?php if ($settingsUpdated !== '') : ?>
                            <div class="wgp-inline-notice wgp-inline-notice--success"><p><?php esc_html_e('Settings saved.', 'guild-progress-widget-for-wow'); ?></p></div>
                        <?php endif; ?>
                    </div>
                ...
                <section class="wgp-admin-panel" data-panel="manual-override" hidden>
                    <div class="wgp-admin-card">
                        <div class="wgp-admin-field-grid wgp-admin-field-grid--list">
                            <div class="wgp-admin-field wgp-admin-field--raids">
                                <span class="wgp-admin-field__label"><?php esc_html_e('Manual Progress Override', 'guild-progress-widget-for-wow'); ?></span>
                                <?php if (empty($selectedRaidSlugs)) : ?>
                                    <p class="description"><?php esc_html_e('Select raids in the Appearance tab first. Manual overrides are shown for the currently enabled raids.', 'guild-progress-widget-for-wow'); ?></p>
                                <?php else : ?>
                                        ...
                                        <?php foreach ($selectedRaidSlugs as $slug) : ?>
                                            ...
                                            <div class="wgp-override-card">
                                                ...
                                                <div class="wgp-raid-option">
                                                    ...
                                                    />
                                                    <label class="wgp-raid-option__label" for="<?php echo esc_attr('wgp-override-enabled-' . $slug); ?>"><?php esc_html_e('Use manual values for this raid', 'guild-progress-widget-for-wow'); ?></label>
                                                </div>
                                                <?php if (!empty($overrideBossNames)) : ?>
                                                    <div class="wgp-override-bosses">
                                                        <div class="wgp-admin-field__label"><?php esc_html_e('Boss Kills By Difficulty', 'guild-progress-widget-for-wow'); ?></div>
                                    ...
                                    <p class="description"><?php esc_html_e('Use this only when Blizzard progression is unavailable or incomplete for selected raids. Manual values override the displayed counts for that raid.', 'guild-progress-widget-for-wow'); ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                        <div class="wgp-admin-actions"><?php submit_button(__('Save Changes', 'guild-progress-widget-for-wow'), 'primary', 'submit', false); ?></div>
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" id="wgp-manual-sync-form" class="wgp-admin-sync-form">
                ...
                <p class="description"><?php esc_html_e('Runs an immediate Battle.net sync for guild progression data. Automatic sync still runs daily.', 'guild-progress-widget-for-wow'); ?></p>
            </form>
        </div>
        <?php
    }
    ...
    public function add_plugin_action_links(array $links): array
    {
        $settingsLink = sprintf(
            ...
            esc_html__('Settings', 'guild-progress-widget-for-wow')
        );
    ...
    private function render_region_field(string $value, string $locale): void
    {
        ...
        ?>
        <div class="wgp-admin-field">
            <label for="wgp-region" class="wgp-admin-field__label"><?php esc_html_e('Region', 'guild-progress-widget-for-wow'); ?></label>
...
if (class_exists('WP_Widget')) {
    class Athlios_Guild_Progress_Widget extends WP_Widget
    {
        public function __construct()
        {
            parent::__construct(
                'wow_guild_progress_widget',
        ...
        public function widget($args, $instance)
        {
            ...
            wp_enqueue_style('wgp-style');
            wp_enqueue_script('wgp-script');
            echo Athlios_Guild_Progress_Plugin::instance()->build_progress_html(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
